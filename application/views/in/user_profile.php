<section>
     <div class="cs_height_100 cs_height_lg_80"></div>
     <div class="container py-5">
         <div class="row cs_gap_y_50">
             <div class="col-lg-4">
                 <form class="" action="<?=base_url()?>in/check_availability" method="post">
                     <div class="row g-3">
                         <div class=" col-md-12">
                         <div class="cs_form_item_btn">
                                 <a href="<?=base_url('in/user_dashboard')?>" class="cs_btn cs_style_1 cs_fs_15 cs_medium cs_radius_5 w-100">
                                    Dashboard
</a>
                             </div>
                         </div>
                         <div class=" col-md-12">
                             <div class="cs_form_item_btn">
                                 <a href="<?=base_url('in/user_profile')?>" class="cs_btn cs_style_1 cs_fs_15 cs_medium cs_radius_5 w-100">
                                    User Profile
</a>
                             </div>
                         </div>
                     </div>
                 </form>

             </div>
             <div class="col-lg-8">
                 <div class="row cs_gap_y_60">
                     <div class=" col-lg-6 col-md-6">
                         <div class="cs_card cs_style_7 cs_radius_5 overflow-hidden">
                             <a href=""
                                 class="cs_card_thumb d-block overflow-hidden position-relative cs_primary_bg">
                                 <img src="http://localhost/Git/moonlight/assets/web/assets/img/room_20.jpg" alt="Room">
                             </a>
                             <div class="cs_card_info position-relative">
                                 <h2 class="cs_card_title cs_fs_50 cs_mb_4"><a
                                         href=""></a></h2>
                                 <div class="cs_card_price cs_mb_17">
                                     <span class="cs_primary_color">From</span>
                                     <span class="cs_accent_color cs_fs_38 cs_primary_font">&#8377;
                                        </span>
                                 </div>
                                 <ul class="cs_card_list cs_mp_0">
                                     <li>110 Sq</li>
                                     <li>3-5 Guests</li>
                                     <li>Free Wi-Fi</li>
                                 </ul>
                             </div>
                         </div>
                     </div>
                     <div class=" col-lg-6 col-md-6">
                         <div class="cs_card cs_style_7 cs_radius_5 overflow-hidden">
                             <a href=""
                                 class="cs_card_thumb d-block overflow-hidden position-relative cs_primary_bg">
                                 <img src="http://localhost/Git/moonlight/assets/web/assets/img/room_20.jpg" alt="Room">
                             </a>
                             <div class="cs_card_info position-relative">
                                 <h2 class="cs_card_title cs_fs_50 cs_mb_4"><a
                                         href=""></a></h2>
                                 <div class="cs_card_price cs_mb_17">
                                     <span class="cs_primary_color">From</span>
                                     <span class="cs_accent_color cs_fs_38 cs_primary_font">&#8377;
                                        </span>
                                 </div>
                                 <ul class="cs_card_list cs_mp_0">
                                     <li>110 Sq</li>
                                     <li>3-5 Guests</li>
                                     <li>Free Wi-Fi</li>
                                 </ul>
                                 
                             </div>
                         </div>
                     </div>
                 </div>
             </div>
         </div>
     </div>
     <div class="cs_height_150 cs_height_lg_80"></div>
 </section>